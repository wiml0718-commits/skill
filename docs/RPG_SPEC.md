...
`unassignedXP` 承接無法歸屬到任何技能的 XP。正常操作不會產生，只用於遷移與邊界
情況，見 §4.3.1。
...
3. 所有 `rewards[].skillId` 指向被移除技能的條目從 `rewards` 陣列中刪除。整個
   `rewards` 因此變空的步驟，之後完成時退回 §4.3 第 2 步（改用所屬 goal 的
   `coreId`）；連 `coreId` 也已清成 `null` 時走 §4.3.1。

即使如此，§4.3 第 2 步仍必須防禦：**承接技能不存在時視同未歸屬**，走
...
`builtin: true`，不可單獨刪除，但會隨其核心一併移除。它存在的唯一理由見 §4.3.1
末段。
...
**`skillId` 可為 `null`**，代表這筆 XP 尚未歸屬到任何技能（§4.3.1）。未歸屬的完成
...
**由 `meta.activeDays` 推導，不由 `xpLog` 推導。** store 在寫入 `source` 為
`step` 或 `manual` 的 `xpLog` 時，同步把該筆的 `date` 併入 `activeDays`（升序、
去重）。補登併入的是被補登的那一天，與 §5.1 一致。

`merge` 與 `rollup` 都不併入。`merge` 只是搬移既有 XP，不是當天推進了什麼；
`rollup` 是壓縮產生的月彙總，它的 `date` 是月初而不是真的有活動的那天，併進去會
在 `activeDays` 裡憑空多出一批假的活動日。
...
判定為純函式 `evaluateAchievements(state, today) -> Set<achievementId>`（`today`
為 §5.0 的邏輯日），完全從
...
5. 既有 v2 解得開但含壞資料時，覆寫前先把原樣寫入 `skill-damaged-v2`（若已
   存在則不覆寫），並在啟動時回報損失筆數。**快照確定留成了才准覆寫**；留不成
   （配額滿、既有快照是更早的另一份）時這次開啟一律不寫入，並明確告知使用者。

沿用現有 `sanitize` 的容錯原則：壞掉的單筆資料跳過，不讓一筆髒資料毀掉整包。
但遷移必須另外回報跳過筆數，不能靜默吞掉——遷移時的靜默丟失是無法察覺的資料
損失。一般載入（沒有遷移）時丟掉的紀錄同樣要回報。
...
以下五項原本是我在規格中先填的預設值，**全部經過人工確認**，規格內文已同步。
前三項改掉了，後兩項維持原值。
...
4. **`xpLog` 保留策略**：**400 天逐筆，更舊者壓成每月彙總**（§3.6）。一年多的逐筆
   細節足夠支撐回顧，更舊的只需要總量；永久保留在 `localStorage` 5MB 上限下有撐爆
   的風險。
5. **成就通知**：**只用 toast，不發系統通知**。解鎖當下使用者本來就在看 App，
   系統通知留給真正需要把人拉回來的到期提醒，不稀釋它。
